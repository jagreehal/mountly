...
import type { McpResourceDeclaration, McpToolVisibility } from "../index.js";
import { MCP_APPS_PROTOCOL_VERSION } from "../index.js";
...
function assertSidecarCompliance(meta: McpResourceDeclaration, htmlPath: string): void {
  if (meta.mimeType !== RESOURCE_MIME_TYPE) {
    throw new Error(
      `mountly-mcp/server: sidecar mimeType must be '${String(RESOURCE_MIME_TYPE)}' (received '${String(meta.mimeType)}') for ${htmlPath}`,
    );
  }
