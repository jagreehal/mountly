...
test.describe("Multi-widget bundles", () => {
  test("createWidgetBundle: one Svelte bundle, three features, one fetch each for JS and CSS", async ({
    page,
  }, testInfo) => {
  ...
  test("Light-DOM bundle: shadcn-flavoured React widgets share global Tailwind/tokens", async ({
    page,
  }, testInfo) => {
  ...
  test("React widget importing @tanstack/react-table works through the mountly pipeline", async ({
    page,
  }, testInfo) => {
