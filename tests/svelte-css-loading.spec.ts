...
test("Svelte adapter applies styles passed as a literal `styles` option (shadow DOM)", async ({ page }) => {
...
test("Svelte adapter fetches CSS via `cssUrl` option and adopts it into the shadow root", async ({ page }) => {
...
test("Svelte adapter derives CSS URL from `moduleUrl` option (.js → .css)", async ({ page }) => {
...
test("Svelte adapter accepts `cssUrl` passed via mount() props", async ({ page }) => {
...
test("Svelte adapter derives CSS from `moduleUrl` passed via mount() props (regression)", async ({ page }) => {
  // Regression: an earlier version fetched the JS bundle as CSS when
  // moduleUrl came in via props, producing an empty stylesheet.
...
test("Svelte adapter sends `Accept: text/css` so dev servers return raw CSS, not JS-wrapped modules", async ({ page }) => {
  // Simulate a Vite-style dev server: the same URL returns either JS
  // (default) or CSS (with `Accept: text/css`). Without the header the
  // adapter would consume JS code and produce an empty stylesheet.
