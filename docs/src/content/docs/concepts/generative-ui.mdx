---
title: Generative UI (advanced)
description: Advanced. Render @json-render specs as MCP Apps Views. Prefer createMcpView for most Apps; use this when the UI itself is model-generated.
---

import { exampleUrl } from "../../../lib/example-urls.ts";

:::caution[Advanced path]
Most MCP Apps should start with [`mountly-mcp create`](/mountly/mcp-apps/) and
`createMcpView`. `mountly-mcp/json-render` is optional. Use it when an agent
**emits the UI structure**, not only the data.
:::

`mountly-mcp/json-render` is the **generative UI** path for Mountly MCP. An AI emits a UI spec
...
(`@json-render/core` + `@json-render/react`), [`mountly-mcp/react`](/mountly/mcp-apps/),
...
Reach for this instead of a hand-built View when you want the **UI itself to be
...
## The View, in one call

`createGenerativeView` collapses catalog → registry → renderer → `$state` →
...
The server tool returns the spec as the tool result, exactly like any other MCP Apps View
(see [MCP Apps](/mountly/mcp-apps/)):
...
// Live: the UI assembles element by element:
...
| ----------------------------------------------------------------------------- | ---------------- | ------------------------------------------------------------------------------------------------- |
| `createGenerativeView(opts)`                                                  | `.`              | catalog + components → MCP View                                                                   |
...
| `useUIStream` / `useChatUI`                                                   | `.`              | **live** client hooks that stream a spec from an endpoint (json-render's, re-exported)            |
...
**Live:** <a href={exampleUrl("mcp-generative-demo/preview/stream-dist/")}>generative UI stream demo</a>
