...
description: The <mountly-feature> element, and how a manifest host renders independently-deployed verticals as tags.
---

:::tip[Sharing your components with another team?]
Use [script-tag component libraries](/mountly/concepts/script-embeds/). You write
an ordinary React, Vue or Svelte component and the consumer writes one script tag
and a typed element. No import map, no registration call, no JSON props.

This page covers `<mountly-feature>`, which is a different job: it is the element
a **host page** uses to render verticals it composes from a manifest, sharing one
framework instance across all of them.
:::

`<mountly-feature>` is declarative markup for a host that composes several
independently-deployed verticals. The host owns the import map and the vertical
list; the element handles attach, mount, prop updates and unmount on disconnect.
...
Most hosts generate that list from a manifest rather than writing it by hand, with
[`defineMountlyFeatureFromManifest`](/mountly/concepts/manifest-hosts/), which is
the intended entry point. See [microfrontends](/mountly/concepts/microfrontends/)
for the whole picture.

## Alias tags

`aliases` maps a browser tag to a module ID, and `prefix` namespaces the
generated ones. Custom element names must contain a hyphen, so a module ID like
`signup` needs an alias:
...
The lower-level `registerCustomElement()` is there when a vertical needs custom
loading, data or analytics wiring.
