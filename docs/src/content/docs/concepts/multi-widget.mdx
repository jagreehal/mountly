...
mountly's default unit is one widget per `dist/index.js`. That fits when widgets are independent. When widgets share code (a design system, utility components, shadcn primitives, a date library), separate bundles force every page that uses two or more to redownload the shared bytes. `createWidgetBundle` collapses that duplication.
...
A test in the repo (`tests/fixtures/tanstack-table-widget.js` plus its spec) loads a third-party package this way and renders it through the mountly pipeline.

## Light DOM vs shadow DOM for a bundle

Commit a whole bundle to **one mode**. Mixing light-DOM and shadow-rooted widgets within a single design system confuses your styling assumptions (does the host's Tailwind reach in? does the design token cascade work?).
...
The repo's `docs/examples/shadcn-light-dom` and matching test (`shadcn-light-dom.html`) demonstrate three React widgets sharing a global Tailwind/tokens stylesheet through this approach.
...
`docs/examples/monorepo-component-library/README.md` walks through the
...
knows nothing about mountly. The team also wants to drop widgets that use
this library onto static pages, third-party hosts, or partner sites.

Add **a separate bridge package** rather than forking the
library:
...
The library stays a normal component library. You can import it wherever
React/Vue/Svelte renders. The bridge is small (tens of lines
...
// + pricing, newsletter, etc. All default to light DOM
```
...
light-DOM mode preserved, bundle JS fetched once.
...
You _can_ have one widget with `shadow: true` and another with `shadow: false` in the same bundle. It rarely ends well: the global stylesheet styles half your widgets and not the others, design-token inheritance goes asymmetric, and every bug report turns into "but on this widget…" detective work. Pick a mode per bundle.
