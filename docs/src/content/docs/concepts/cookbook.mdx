---
title: Cookbook
description: "Common patterns for using mountly in production: forms, auth, navigation, loading states, and error handling."
---
...
A widget needs to navigate the host page. Use the event bus: the widget emits a navigation event and the host listens.
...
The data fetch runs in parallel with the module fetch, and the result is cached by `paymentId`. Hover a second payment and you get a cache hit.
...
function WidgetErrorBoundary({ children }: { children: React.ReactNode }) {
  ...
  if (error)
    return (
      <div class="error">
        Something went wrong. <button onClick={() => setError(null)}>Retry</button>
      </div>
    );
  return (
    <ErrorBoundary
...
A single feature can mount into multiple containers, which covers the same widget in a sidebar and a modal:
