...
is what makes it fast — no second bootstrap, one copy of React — and it is also
...
untrusted verticals can be framed while the other twenty stay on import maps —
...
may print *"The above dynamic import cannot be analyzed"* once when it processes
...
gets blocked by CSP, throws before `mountAsFrame`, or simply forgets to load
...
mid-load — a route change, say — does not report a failure afterwards.
...
fetched — the wins available are *early* bytes and *repeat-visit* bytes.
...
separate, separately-cached files means a deploy invalidates only what actually
changed.
