...
`auto.js` (2.3 KB gzipped) wires every island on the page. It also picks up
islands that another widget, an htmx swap or a Turbo frame adds later, so
...
| `data-mountly-state` | Set by mountly     | `idle` / `loading` / `mounted` / `error`, styled with CSS                      |
