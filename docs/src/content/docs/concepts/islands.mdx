...
| `trigger`          | `click` in the feature layer         | Activation trigger; `never` leaves the SSR island inert.                       |
...
Set `forceRemount: true` when the island is marked hydrated but you want a client remount anyway.
...
**Use when:** You want the payload to say “this has SSR content” while forcing the client version to replace it. mountly warns about this path, since mismatched server and client output can cause flicker.
...
- **Custom elements have form constraints (custom element approach only).** When you register custom elements with `defineMountlyFeature()`, form integration differs. Use the dependency approach with direct `mountIslandFeature()` calls if you need native form behavior.
