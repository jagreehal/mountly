...
URL routing across independently deployed verticals is covered — [`createFeatureRouter`](/mountly/concepts/microfrontends/#routing-across-verticals) maps segments to features and each vertical routes below its own segment. What mountly still does not do is coordinate *deployments*: it validates manifests and loads verticals, but it will not resolve semver at runtime, gate a rollout, or hold a vertical back until another is ready. If you need that, use single-spa or a vendor MFE platform.
