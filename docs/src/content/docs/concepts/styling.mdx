...
mountly's styling model has one design goal: **make the right thing the easy thing.** The default path takes zero config and works the same way for React, Vue, and Svelte. Reach for overrides when you need them.
...
That's the entire pipeline. The host writes no `render` function. The widget loads no CSS by hand. Vite's library build emits the sibling `.css` for you. The widget renders inline, so the host's design system reaches in.
