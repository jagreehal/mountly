...
The default path needs no styling configuration and behaves the same on React, Vue, and Svelte: the adapter fetches the widget's sibling stylesheet and applies it before render. Everything below is an override for when that is not what you want.
...
The host writes no `render` function and the widget loads no CSS by hand, because Vite's library build emits the sibling `.css`. The widget renders inline, so the host's design system reaches in.
...
If a fetch fails (404, network error, CORS), the mount **still succeeds**: the widget renders unstyled rather than throwing. mountly logs the failure without interrupting. If you want hard failures, validate the asset at build time.
...
When you embed a widget in unknown hosts (CMS pages, third-party sites, legacy apps with conflicting CSS), opt into shadow DOM:
...
CSS Modules pair with both modes. The build emits hashed class names, the JSX or template references them, and mountly does the wiring.
...
In light DOM the hashed class names prevent host conflicts in most cases. Add `shadow: true` if you also need to keep the host's CSS out. The repo has a Playwright test (`react-css-modules.html`) that proves the shadow case with a deliberate decoy rule.
...
Vite's dev server returns `.css` files as **JS modules** by default. The module exports a string of CSS _and_ registers itself with HMR. If you `fetch("/widget.css")`, you get JavaScript:
...
Vite's content-negotiation returns raw CSS for that header, so the stylesheet parses. This matters only if you put a custom proxy or service worker between the page and Vite.

## Islands and noscript

When JavaScript is disabled, no adapter runs. There is no shadow root, no `adoptedStyleSheets`, no injected `<style>`. The browser renders whatever HTML and `<link>` tags the server sent.
...
<body>
  <div data-mountly="/widgets/card/dist/index.js">
...
- **JS off**: the `<link>` styles the server-rendered span. The script never runs. The user sees styled content.
- **JS on**: the host bootstraps. On click, the island hydrates. By default it mounts in light DOM and the document `<link>` continues to apply. If the island also passes `shadow: true`, the adapter adopts the same CSS into the shadow root, which scopes it; the document-level `<link>` is then duplication that costs you nothing.
...
Adding a `<link>` requires JavaScript. If JS is off, no script runs, including mountly's host entry. The only thing that works without JS is server-rendered HTML. The noscript pattern is a **content-author** decision: ship the SSR markup _and_ the `<link>` alongside it.
