...
A widget knows nothing about *when* it loads. It knows how to render into a container and how to clean itself up. By default it mounts in light DOM so the host's design system applies; pass `shadow: true` to mount inside a shadow root for hard style isolation.
