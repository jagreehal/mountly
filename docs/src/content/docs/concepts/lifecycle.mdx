...
Every feature is a small state machine with five states. It keeps the same shape on every framework and hides no side effects.
...
Splitting `preload` from `activate` lets the trigger choice express intent:

- **Hover** preloads but doesn't activate, since the user might leave.
- **Click** activates regardless of preload state. If preload didn't happen, activation does both.

When the click arrives after hover has finished, mount happens within a frame because the module and (often) data are already cached.
...
If the widget module exposes an `update(container, props)` function, `feature.update(container, props)` calls it and preserves framework-internal state (React reconciliation, Vue reactivity, Svelte component instance).
...
The custom element calls this when its `props` attribute changes.
...
If `loadData` rejects, the same recovery applies. The module stays cached, and mountly refetches data on the next attempt.
