...
When a framed vertical needs first-party cookies or relative asset URLs under the host origin, `createSameOriginProxy` from `mountly-manifest/server` forwards matching **GET/HEAD** requests to an upstream. It is never required for embed — CDN + CORS is the default path. See [Same-origin proxy](/mountly/concepts/same-origin-proxy/).
