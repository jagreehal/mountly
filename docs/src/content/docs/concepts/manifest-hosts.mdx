...
When `isolation` is `"iframe"`, `defineMountlyFeatureFromManifest` registers the vertical with `iframeFeature`. See [When to frame](/mountly/concepts/when-to-frame/), [Frame protocols](/mountly/concepts/frame-protocols/), and [Same-origin proxy](/mountly/concepts/same-origin-proxy/).
...
`mountlyManifestFragmentPlugin` fills `types` for you by asking the TypeScript
checker what each entry exports, so re-exports and `export * from` are included
and type-only exports are not. `types.module` lists what a host can import at
runtime.
...
Runnable reference: <a href={exampleUrl("vite-host-import/")}>vite-host-import live demo</a> · [source](https://github.com/jagreehal/mountly/tree/main/docs/examples/vite-host-import).
...
When a framed vertical needs first-party cookies or relative asset URLs under the host origin, `createSameOriginProxy` from `mountly-manifest/server` forwards matching **GET/HEAD** requests to an upstream. Embedding never requires it; CDN plus CORS is the default path. See [Same-origin proxy](/mountly/concepts/same-origin-proxy/).

## Related

- [Microfrontends](/mountly/concepts/microfrontends/): composition patterns and custom elements
- [When to frame](/mountly/concepts/when-to-frame/): manifest flip to `isolation: "iframe"`
