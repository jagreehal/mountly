...
mountly keeps two cooperating caches. Both live in memory and survive re-mounts within a page session.
...
You rarely touch it directly. Use the exported `moduleCache` to seed entries or invalidate from outside the runtime:

```ts
import { moduleCache } from "mountly/cache";
...
```ts
import { dataCache } from "mountly/cache";
...
```ts
import { DedupCache } from "mountly/cache";

const cache = new DedupCache<MyValue>();

const value = await cache.resolve("key", async () => fetchSomething(), { signal });
```
...
Open the same widget elsewhere on the page and both caches hit, so mount happens within a frame.

## Inspecting the cache

The [devtools panel](/mountly/concepts/devtools/) shows live module and data cache contents alongside feature states. Use it to spot an over-aggressive prefetch or a missed cache-key collision.
