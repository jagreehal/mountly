...
TkDodo argues for teams that own a slice top to bottom: frontend, backend, design, all in one team. mountly fits this and adds one more axis. The _framework_ can also be a team choice.
...
Shadow DOM is one tool you can reach for. The vertical stays the discipline you keep.

## Verticals as packages

When a Feature stabilises and a second consumer picks it up, promote it to its own package. The usage shape stays the same:
...
The vertical codebase is a layout discipline. mountly makes you enforce it at runtime: one public surface, one runtime contract, and internals that stay internal.
