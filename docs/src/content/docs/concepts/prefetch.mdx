...
```ts
import { createPredictivePrefetcher, recordInteraction } from "mountly/prefetch";
...
```ts
import { createScrollPrefetcher } from "mountly/prefetch";
...
```ts
import { createMouseTrailPrefetcher } from "mountly/prefetch";
