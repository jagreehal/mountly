...
`feature.update()` patches props in place, so the component instance survives and its local state with it.

- **Svelte 4**: the adapter calls `$set` on the instance.
- **Svelte 5, adapter-loaded runtime**: the adapter hands Svelte a `$state`-tracked props object and assigns onto it. A prop the host drops clears to `undefined` rather than lingering from the last render.
- **Svelte 5, your own `mount`/`unmount`**: there is no tracked object to patch, so the adapter assigns onto the mount handle. That reaches the component's exported bindings only. Omit `mount`/`unmount` if you want prop updates to flow.
...
To hand a Svelte component to a team that does not use Svelte, point a build at it and give it a namespace. Mountly reads `let { … }: Props = $props()` and publishes the component as a custom element:

```ts title="vite.config.ts"
import { defineElementsConfig } from "mountly-vite-plugin";

export default defineElementsConfig({ prefix: "acme", elements: "src/elements/*.svelte" });
```

The consuming page needs one script tag and no package install. See [Script-tag component libraries](/mountly/concepts/script-embeds/).
