...
The signature is the same across adapters by design.

```ts
function createWidget<P>(Component: ComponentType<P>, options?: AdapterOptions): WidgetModule;
```
...
The React adapter exposes `update()`. `feature.update(container, props)` reuses the same React root and preserves hooks state. **CSS Modules** fit the default light-DOM mount: the build emits hashed class names referenced from JSX, and the host's design system can still reach in. For full isolation, add `shadow: true`. See [Styling](/mountly/concepts/styling/).
...
The adapter auto-loads `mount` / `unmount` from `"svelte"` if you don't supply them, but passing them avoids a deferred dynamic import on first mount.
...
You write no manual CSS loading. The adapter loads each widget's companion CSS and applies it before the widget renders, so you get no FOUC and no host conflicts.
...
The adapter caches fetched CSS in memory per URL, so repeated mounts of the same widget never refetch. See [Styling](/mountly/concepts/styling/) for the full matrix including light DOM, CSS Modules, and noscript fallback patterns.

## Light DOM (default)

By default `createWidget` mounts in light DOM. The widget renders directly inside the container, the host's CSS reaches in, and the widget's `styles` are injected as a single `<style data-mountly-fallback>` in `<head>` (or inline next to the mount with `styleMode: "isolated"`). This is the right default for most product code, where you want your design system to apply.
