...
description: The core primitive that wraps a widget with the on-demand lifecycle.
---
...
Builds an [`OnDemandFeature`](#ondemandfeature): a widget plus its lifecycle, caching, and trigger setup. You wire this layer to the page.

## Signature

```ts
function createOnDemandFeature(options: CreateOnDemandFeatureOptions): OnDemandFeature;
```
...
The high-level wiring. It sets up the trigger, calls `mount` / `unmount` for you, and returns a cleanup function.
...
Begins the module fetch (and `loadData` if you pass `ctx`). It's idempotent: calling it twice while preloading shares one promise.
...
Aborts any in-flight `preload` / `activate`. State transitions to `"aborted"`. The next `preload` / `activate` / `mount` recovers on its own.
...
```ts
type FeatureState =
  "idle" | "preloading" | "preloaded" | "activating" | "activated" | "mounted" | "aborted";
```
...
For the common case (load a widget bundle and let its companion CSS apply on its own), pass `moduleUrl` and you're done:
...
mountly synthesises `loadModule` (via `createModuleLoader`) and a default `render` that forwards `moduleUrl` into the adapter's mount props. The adapter fetches `dist/index.css` (sibling of the JS bundle), sends `Accept: text/css` so dev servers like Vite return raw CSS rather than HMR-wrapped JS, and applies it before render (injected globally for the default light-DOM mount, or adopted into the shadow root when `shadow: true`). See [Styling](/mountly/concepts/styling/) for the full picture.
