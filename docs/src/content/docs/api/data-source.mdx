...
Widgets render from `status`, `loading`, `error`, and `data` rather than each framework inventing a different shape.
...
Call `abort()` when an island stops mattering, such as a route transition or a closing modal. Call `invalidate()` when a mutation changes server state.
...
const quoteSource = createDataSource<string, Quote>({
  staleTime: 60_000,
  load: ({ key, signal }) => fetch(`/api/quotes/${key}`, { signal }).then((r) => r.json()),
});
