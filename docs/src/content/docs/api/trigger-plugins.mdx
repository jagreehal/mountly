...
The five built-in triggers (`hover`, `click`, `focus`, `viewport`, `idle`) cover most cases. For the rest, mountly ships a small **trigger plugin** API and four ready-made plugins.
...
```ts
const cleanup = createSwipeTrigger(panelElement, () => panelFeature.activate(), {
  direction: "left",
  threshold: 60,
});
```
...
```ts
const cleanup = createLongPressTrigger(contextMenuTarget, () => menuFeature.activate(), {
  duration: 600,
  tolerance: 8,
});
```
...
```ts
const cleanup = createUrlChangeTrigger(pageContainer, () => billingFeature.activate(), {
...
registerTriggerPlugin({
  ...
  setup(element, onTrigger, options) {
    ...
    const handler = () => {
      const now = Date.now();
      if (now - lastTap < (options?.window ?? 300))
        onTrigger({ element, triggerType: "programmatic" });
      lastTap = now;
    };
