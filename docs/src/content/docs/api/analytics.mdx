...
There is no built-in per-feature filter. Filter in your callback.
...
Subscribers see all events emitted via `emitAnalyticsEvent`. There's no schema validation, so use it sparingly.

## `createFeatureTimingTracker(moduleId)`

The internal helper that `createOnDemandFeature` uses. It's exposed for one advanced case: building a custom feature shape that joins the same analytics pipeline.
...
You'll rarely call this directly.
