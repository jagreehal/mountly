...
<payment-breakdown trigger="idle" props='{"invoiceId":"inv_123"}'></payment-breakdown>
...
```ts
function defineMountlyFeature(input?: string | DefineMountlyFeatureOptions): void;
...
## Trigger → core mapping

The element is a shim: it translates its attributes into the core's `data-on`
string and hands the element to `wire()`.

| Attributes                                    | `data-on`           |
| --------------------------------------------- | ------------------- |
| `trigger="click"` (default)                   | `click`             |
| `trigger="hover"`                             | `hover`             |
| `trigger="focus"`                             | `focus`             |
| `trigger="viewport"` + `viewport-root-margin` | `viewport:<margin>` |
| `trigger="idle"` + `idle-timeout`             | `idle:<ms>`         |
| `trigger="media"` + `activate-media-query`    | `media:<query>`     |
| `trigger="url-change"`                        | `url`               |

`preload-on` maps the same way into `data-preload` and is never derived from
`trigger`, because preloading on the event that also mounts is a no-op. An unrecognised
`trigger` falls back to `click`. Hover has no delay attribute; use the core's
`data-on="hover:300"` for that.

If you need combinations the element can't express, use
[`feature.attach(...)`](/mountly/api/create-on-demand-feature/#attachoptions) directly.
...
Setting the `props` attribute on a connected element triggers `feature.update(container, parsedProps)`, which preserves framework-internal state when the widget supports it.
