---
title: URL state
description: Parse, serialize, and patch query parameters from several islands without them clobbering each other.
---
...
The URL state helper stays small on purpose. It handles query strings only: parse current params, serialize deterministic output, and write patches without dropping unrelated keys owned by another island.
