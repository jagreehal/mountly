...
<Mermaid
  title="Decision ladder: start by asking whether you can merge into a monorepo or publish an npm package. If yes, use a monorepo with a shared library and lazy routes. If no, ask whether the host page is one you do not own. For a CMS, static, or legacy page, use a mountly widget as a custom element drop-in. If you own the shell, ask whether regions share one Router, Query, or Redux graph. Keep one app shell when they do. Otherwise, ask whether teams need independent CDN deploys. Use mountly as an npm dependency when they do not, or a mountly manifest with import-map verticals when they do. Add Single-SPA or a vendor MFE platform when you also need route-to-app orchestration, staged rollouts, or runtime release coordination."
  chart={`flowchart TD
...
team at runtime — which quietly costs you the independent deployability the
split was for. If that describes your codebase, the browser will enforce the
boundary for you:
...
frame document at preload time — hover, viewport, idle — which buys back the
network half but not the runtime half.
