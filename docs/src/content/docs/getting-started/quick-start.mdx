...
The widget mounts inside its own shadow root. Styles are bundled and scoped — the host can't bleed into the widget and the widget can't bleed out.
...
const signup = createOnDemandFeature({
  ...
  loadModule: () => import("./my-widget/dist/index.js"),
...
Now the widget code is fetched on first hover and mounted on click. Open DevTools → Network and watch.
