...
> mountly turns React, Vue, and Svelte components into lazy, HTML-addressable features (light DOM by default; `shadow: true` for isolation) that can be dropped into CMS pages, marketing sites, legacy apps, partner embeds, and framework migrations.
...
Teams use that primitive in several architectures.

That primitive can form a lightweight microfrontend composition layer. A team-owned feature becomes a microfrontend when its team builds, tests, and deploys it on a release path separate from the host. mountly supplies the browser contract and loading machinery. Your CDN and release process supply the deployment boundary.
...
| Multi-team sites      | Each team has a separate release path. The host manifest pins versions.                      |
| Cross-framework pages | React, Vue, and Svelte widgets coexist on one page. Each team picks their framework.         |
| MCP-hosted UI         | mountly components wrapped as MCP App Views. Same component, two activation contexts.        |
...
- A lightweight composition layer for widgets and regions with separate release paths.

## Scope

- Next, Nuxt, SvelteKit, Remix, or Astro can own the application shell and SSR.
- The host application owns full-app routing and global state.
- Your delivery platform coordinates staged rollouts, traffic, and runtime version policy.
- Components keep their React, Vue, or Svelte authoring model.
...
mountly uses custom elements as a delivery mechanism, not as the authoring model. You still write React, Vue, or Svelte components. The adapter exposes them as mountable widgets, and mountly gives the host a stable tag such as `<signup-card>`.
