...
You can add [custom triggers](/mountly/api/trigger-plugins/) such as swipe, long-press or a keyboard chord with a single assignment, without touching the runtime.
...
Includes the component, the adapter, and the framework. Drop it into any page and it works without host setup. A representative React widget weighs about 66 KB gz.

### `dist/peer.js`, peer build

Excludes the framework. The host provides `react`, `react-dom`, `react-dom/client`, and `react/jsx-runtime` via an import map. Each widget weighs about 3 KB gz, plus one 59 KB gz copy of React shared across all of them.
