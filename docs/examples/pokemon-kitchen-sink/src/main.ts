...
const pokemonFeature = createOnDemandFeature({
  ...
  loadData: async (ctx) => {
    const name = (ctx.pokemonName as string | undefined) ?? "";
  ...
  getCacheKey: (ctx) =>
    `pokemon:${(ctx.pokemonName as string | undefined) ?? (ctx.dataUrl as string | undefined) ?? ""}`,
...
const customElementPokemonFeature = createOnDemandFeature({
  ...
  loadModule: async () => (await import("./pokemon-detail-module")).default,
  loadData: async (ctx) => {
    const url = (ctx.dataUrl as string | undefined) ?? "";
    const response = await fetch(url);
