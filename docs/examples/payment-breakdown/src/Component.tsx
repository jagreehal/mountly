...
export function PaymentBreakdown({ data, onClose }: PaymentBreakdownProps) {
  ...
  return (
    ...
    >
      ...
      <ol className="mt-4 space-y-1.5 text-sm" style={{ color: "var(--ui-text-subtle)" }}>
        {data.items.map((item, i) => (
          <li key={i} className="flex items-baseline gap-4">
            <span className="min-w-0 flex-1 truncate">{item.description}</span>
            <span className="tabular-nums" style={{ fontFamily: "var(--ui-font-numeric)" }}>
      ...
      {hasAdjustments && (
        <>
          <div className="my-3 h-px" style={{ background: "var(--ui-border)" }} aria-hidden />
          <dl className="space-y-1 text-sm" style={{ color: "var(--ui-text-subtle)" }}>
            <div className="flex items-baseline gap-4">
              <dt className="flex-1">Subtotal</dt>
              <dd className="tabular-nums" style={{ fontFamily: "var(--ui-font-numeric)" }}>
            ...
            {data.discount !== undefined && (
              <div className="flex items-baseline gap-4" style={{ color: "var(--ui-positive)" }}>
      ...
      <div className="mt-4 border-t pt-3" style={{ borderColor: "var(--ui-rule)" }}>
