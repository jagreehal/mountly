...
import { onAnalyticsEvent, getAnalyticsLog } from "mountly/analytics";
import {
...
attach(paymentBreakdown, {
  ...
  context: { paymentId: "pay_123", apiUrl: paymentApiUrl },
...
const multiTrigger1 = document.querySelector<HTMLElement>('[data-testid="multi-trigger-1"]')!;
const multiPopover1 = document.querySelector<HTMLElement>('[data-testid="multi-popover-1"]')!;
const multiTrigger2 = document.querySelector<HTMLElement>('[data-testid="multi-trigger-2"]')!;
const multiPopover2 = document.querySelector<HTMLElement>('[data-testid="multi-popover-2"]')!;
...
attach(paymentBreakdown, {
  ...
  context: { paymentId: "pay_456", apiUrl: paymentApiUrl },
});

// ── 1c. Host-reserved space + host-owned skeleton ─────────────────────
const skeletonTrigger = document.querySelector<HTMLElement>('[data-testid="skeleton-trigger"]')!;
const skeletonPopover = document.querySelector<HTMLElement>('[data-testid="skeleton-popover"]')!;
attach(paymentBreakdown, {
...
const updatePopover = document.querySelector<HTMLElement>('[data-testid="update-popover"]')!;
...
updateMountBtn.addEventListener("click", async () => {
  ...
  updateMountHandle = await paymentBreakdown.mount(updatePopover, {}, initialUpdateProps);
});
updateChangeBtn.addEventListener("click", () => {
  updateCounter += 1;
  const total = 10 + updateCounter * 5;
  void paymentBreakdown.update(updatePopover, {
    data: {
      total,
      currency: "USD",
      items: [{ description: `Seat ×${1 + updateCounter}`, amount: total, currency: "USD" }],
    },
  });
});
...
const lightboxTrigger = document.querySelector<HTMLElement>('[data-testid="lightbox-trigger"]')!;
const lightboxMount = document.querySelector<HTMLElement>('[data-testid="lightbox-mount"]')!;
const SAMPLE_SVG =
  ...
  ceFeature.setAttribute(
    ...
    JSON.stringify({
      data: {
        total: 250,
        currency: "GBP",
        items: [{ description: "Updated plan", amount: 250, currency: "GBP" }],
      },
    }),
  );
});
...
const prefetcher = createPredictivePrefetcher({
  features: [{ feature: paymentBreakdown, weight: 3, triggers: ["hover", "click"] }],
  strategy: "staggered",
  staggerDelay: 300,
});
