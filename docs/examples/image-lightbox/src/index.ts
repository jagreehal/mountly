...
export const imageLightbox = createOnDemandFeature({
  ...
  render: ({ mod: featureMod, container, props }) => {
    const data = props.data as ImageLightboxData | undefined;
    if (!data) {
      throw new Error("[image-lightbox] no data: pass { src, alt, caption? } as props.data");
    }
...
export { mountImageLightbox, unmountImageLightbox, updateImageLightbox } from "./mount.js";
