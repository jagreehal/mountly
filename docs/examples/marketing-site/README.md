...
Open <http://localhost:5175/docs/examples/marketing-site/>.
...
2. **Inline "Try it inline"**: declarative `<mountly-feature>`, viewport-triggered. Mounts when you scroll it into view. Shows retheming via `--primary`/`--ring` overrides on the mount element.
...
3. They paste or merge their UI into `docs/examples/signup-card/src/Component.tsx` (or their own package path).
...
5. Marketing drops this HTML snippet anywhere:
   ...
   In this monorepo during local dev, the import map in `index.html` points those entries at `/packages/mountly/dist/*.js` and../docs/examples/signup-card/dist/peer.js` instead.

The host page ships zero framework. The widget runs full React in its shadow root.
