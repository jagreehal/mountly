...
import { streamSpec as streamSpecCore } from "mountly-mcp/json-render/server";
...
async function resolveModel(opts) {
  ...
  if (provider === "mistral") {
    ...
    return {
      model: createMistral({ apiKey: process.env.MISTRAL_API_KEY })(id),
      label: `mistral:${id}`,
    };
  }
 ...
 * (prompt → patches → spec → repair) lives in `mountly-mcp/json-render/server`.
 *
