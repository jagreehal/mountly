import { createRenderer, defineComponents } from "mountly-mcp/json-render";
...
export const components = defineComponents(catalog, {
  ...
  Card: ({ element, children }) => (
    <section className={`gv-card${element.props.accent ? " gv-card--accent" : ""}`}>
      {element.props.title ? <h3 className="gv-card-title">{element.props.title}</h3> : null}
      {children}
    </section>
  ),
  Heading: ({ element }) => <h2 className="gv-heading">{element.props.text}</h2>,
  Text: ({ element }) => (
    <p className={`gv-text${element.props.muted ? " gv-text--muted" : ""}`}>{element.props.text}</p>
  ),
  Stat: ({ element }) => {
  ...
  Sparkline: ({ element }) => {
    ...
    const color =
      tone === "positive" ? "var(--pos)" : tone === "danger" ? "var(--neg)" : "var(--accent)";
